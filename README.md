...
収録は北海道内の全市町村（179）、青森県内の全市町村（40）、東京都内の全区市町村（62）、神奈川県内の全市町村（33）、沖縄県内の全市町村（41）。見出しの団体名から切り替えられる。ヘッダー右の切り口で、1年の歳入→歳出、歳入の時系列、歳出の時系列を切り替える。
...
| パラメータ | 内容 |
| --- | --- |
| `id` | [全国地方公共団体コード](https://www.soumu.go.jp/denshijiti/code.html)（6桁）。札幌市は `011002`、青森市は `022012`、千代田区は `131016`、横浜市は `141003`、那覇市は `472018` |
| `year` | 西暦の決算年度。データに無い年はいちばん近い年度へ寄せる。時系列ビューでも残し、年度ビューに戻ったときに使う |
| `view` | `year`（既定・1年の歳入→歳出）、`revenue`（歳入の時系列）、`expenditure`（歳出の時系列）。既定のときは URL から省く |

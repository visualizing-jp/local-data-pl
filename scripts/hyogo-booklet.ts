/** 兵庫県「財政状況資料集」。リンク文言は「リンク（エクセル）」だけで、団体名は直前のセル。令和4・5年度は隣列に公会計 Excel があるので、最初の xlsx だけ取る。 */
...
/** 県の資料集ページから、団体コード → Excel URL。団体名セルと隣の最初の Excel リンクを組にする。 */
export function parseHyogoBooklet(html: string, pageUrl: string, govs: readonly LocalGov[]): Map<string, string> {
  ...
  for (const match of html.matchAll(TD)) {
    ...
    if (href != null) {
      ...
      if (code != null && allowed.has(code) && !found.has(code)) found.set(code, absUrl(pageUrl, href));
      pending = null;
      continue;
    }

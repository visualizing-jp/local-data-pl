...
function excelJobs(gov: LocalGov): { year: number; path: string }[] {
  ...
  if (gov.prefecture === "奈良県") {
    return Object.keys(NARA_BOOKLET_PAGES).map((year) => ({
      year: Number(year),
      path: resolve(RAW_DIR, gov.code, `${year}.xlsx`),
    }));
  }
